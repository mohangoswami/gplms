...
<div class="container mt-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        @endphp
        <div>
            <a href="{{ route('teacher.attendance.month', ['class' => $class, 'year' => $prev->year, 'month' => $prev->month]) }}" class="btn btn-outline-primary">&larr; Prev</a>
            <a href="{{ route('teacher.attendance.month', ['class' => $class, 'year' => $next->year, 'month' => $next->month]) }}" class="btn btn-outline-primary">Next &rarr;</a>
        </div>
        <div>
            <form method="get" action="{{ route('teacher.attendance.month', ['class' => $class, 'year' => $year, 'month' => $month]) }}">
                <input type="month" name="monthpicker" value="{{ sprintf('%04d-%02d', $year, $month) }}"
                    onchange="(function(el){var parts=el.value.split('-');var base='{{ route('teacher.attendance.month', ['class' => $class, 'year' => 'YEAR', 'month' => 'MONTH']) }}';var url=base.replace('YEAR', parts[0]).replace('MONTH', parts[1]);location=url;})(this)">
            </form>
        </div>
    </div>
